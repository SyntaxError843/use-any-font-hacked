...
function uaf_save_options(){
	...
	/** Check for regenerate font entries HACK */
	if ( ! empty( $_POST['uaf_regenerate_font_entries'] ) && ! empty( absint( $_POST['uaf_regenerate_font_entries'] ) ) ) {

		/** Initialize font files entries */
		$font_entries = [];

		/** List all available woff files in the upload dir */
		$upload_dir = uaf_path_details()['dir'];

		/** Loop through upload dir files */
		foreach ( scandir( $upload_dir ) as $index => $dir_file ) {

			/** Check if file ends with woff */
			if ( substr( $dir_file, -strlen( 'woff' ) ) === 'woff' ) {

				/** Get font path without extension */
				$font_path = str_replace( '.woff', '', $dir_file );

				/** Get font name without any numbers */
				$font_name = strtolower( preg_replace('/[0-9]+/', '', $font_path) );

				/** Add entry */
				$font_entries[ date( 'ymdhis', strtotime("+$index hours") ) ] = array(
		...
		/** Update font entries in database */
		update_option( 'uaf_font_data', json_encode( $font_entries ) );
